...
{if eq( ezini('ModuleSettings', 'Login', 'module.ini'), 'openid')}
<div class="block">
<label for="id3">{"Or use your OpenID to sign in"|i18n("design/ezwebin/user/login")}</label><div class="labelbreak"></div>
<input class="halfbox" type="text" name="OpenIDURL"  value="{$User:openid_url|wash}" />
</div>
{else}
<div class="block">
<iframe src="http://{ezini('ModuleSettings', 'RPXURL', 'module.ini')}/openid/embed?token_url=http://{ezini('SiteSettings','SiteURL')}{"user/login"|ezroot(no)}"  scrolling="no"  frameBorder="no"  allowtransparency="true"  style="width:400px;height:240px;"></iframe> 
{* or replace this whole iframe with your own rpx widget, but remember to set the token_url to your /user/login page on your site *}
</div>
{/if}
